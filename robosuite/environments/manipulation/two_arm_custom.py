...
class TwoArmCustom(TwoArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            arm0_grasp_any, arm1_grasp_cube, cube_height, table_height = self._get_task_info()
            ...
            dist0 = np.linalg.norm(gripper0_site_pos - cube_pos)
            dist1 = np.linalg.norm(gripper1_site_pos - cube_pos)
            # First, we'll consider the cases if the hammer is lifted above the threshold (step 3 - 6)
            if cube_height - table_height > self.height_threshold:
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            @sensor(modality=modality)
            def gripper0_to_cube_pos(obs_cache):
                return (
                    obs_cache["cube_pos"] - obs_cache[f"{pf0}eef_pos"]
                    if "cube_pos" in obs_cache and f"{pf0}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            @sensor(modality=modality)
            def gripper1_to_cube_pos(obs_cache):
                return (
                    obs_cache["cube_pos"] - obs_cache[f"{pf1}eef_pos"]
                    if "cube_pos" in obs_cache and f"{pf1}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            sensors = [cube_pos, cube_quat, gripper0_to_cube_pos, gripper1_to_cube_pos]
    ...
    def _get_task_info(self):
        ...
        arm0_grasp_any = self._check_grasp(gripper=g0, object_geoms=self.cube)
        arm1_grasp_cube = self._check_grasp(gripper=g1, object_geoms=self.cube)

        # Return all relevant values
        return arm0_grasp_any, arm1_grasp_cube, cube_height, table_height
    ...
    def _check_success(self):
        ...
        return (
            True
            if arm1_grasp_cube and not arm0_grasp_any and cube_height - table_height > self.height_threshold
            else False
        )
    ...
    def _gripper1_site_pos(self):
        ...
        Returns:
            np.array: (x,y,z) position of body
        """
        return self.sim.data.site_xpos[self.robots[1].eef_site_id]
